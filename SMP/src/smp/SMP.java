...
import otr.crypt.MPI;
import otr.crypt.Provider;
import otr.crypt.SHA256;
import otr.crypt.SecureRandom;
import otr.other.InBuf;
import otr.other.OTRException;
import otr.other.OutBuf;
import otr.other.Util;
...
public class SMP {
	...
	public static boolean checkGroupElem(MPI g, Provider prov)
	{
	    ...
	    if (prov.compareMPI(g, new MPI(two)) < 0 || 
	    		prov.compareMPI(g, new MPI(SMP.MODULUS_MINUS_2)) > 0){
	    	return true;
	    }
	    return false;
	}
	...
	public static boolean checkExpon(MPI x, Provider prov)
	{
	    ...
	    if (prov.compareMPI(x,new MPI(one))<0 ||
	    		prov.compareMPI(x, new MPI(SMP.ORDER_S))>=0) {
	    	return true;
	    }
	    return false;
	}
