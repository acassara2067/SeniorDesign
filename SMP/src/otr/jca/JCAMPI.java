 ...
 */
public class JCAMPI extends otr.crypt.MPI{
	
    public JCAMPI(byte[] value) {
		super(value);
	}
    
    public static otr.crypt.MPI toMPI(BigInteger val) throws OTRException{
    	byte[] v = toBytes(val);
    	otr.crypt.MPI ret = 
    		otr.crypt.MPI.readMPI(new InBuf(v));
    	return ret;
    }
    
    public static BigInteger getBigInteger(otr.crypt.MPI mpi){
