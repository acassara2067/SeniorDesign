 ...
 */
public class JCAProvider extends otr.crypt.Provider {
	...
	(otr.crypt.MPI p, 
			otr.crypt.MPI q, 
			otr.crypt.MPI g, 
			otr.crypt.MPI y) throws Exception {
		return new JCADSAPublicKey(p, q, g, y);
	}

	public DHPublicKey getDHPublicKey(otr.crypt.MPI val) {
	...
	public int compareMPI(otr.crypt.MPI ours,
			otr.crypt.MPI theirs) {
